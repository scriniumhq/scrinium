...
func (m *fakeMeta) GetMeta(key string) (string, error) {
	v, ok := m.data[key]
	if !ok {
...
func (m *fakeMeta) SetMeta(key, value string) error {
	m.data[key] = value
	m.writes++
	return nil
}
...
func TestDescriptorCache_RoundTrip(t *testing.T) {
	...
	if err := saveDescriptorCache(meta, src); err != nil {
		t.Fatalf("save: %v", err)
	}
	got, err := loadDescriptorCache(meta)
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if got == nil {
...
func TestDescriptorCache_PartialIsCorruption(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			meta := newFakeMeta()
			tc.set(meta)
			_, err := loadDescriptorCache(meta)
			if err == nil {
...
func TestDescriptorCache_SaveOverwrites(t *testing.T) {
	...
	if err := saveDescriptorCache(meta, a); err != nil {
		t.Fatal(err)
	}
	if err := saveDescriptorCache(meta, b); err != nil {
		t.Fatal(err)
	}
...
func (m *errMeta) GetMeta(string) (string, error) { return "", m.err }
func (m *errMeta) SetMeta(string, string) error   { return m.err }

func TestDescriptorCache_LoadPropagatesIOError(t *testing.T) {
	sentinel := errors.New("disk on fire")
	_, err := loadDescriptorCache(&errMeta{err: sentinel})
	if err == nil {
...
func TestDescriptorCache_SavePropagatesIOError(t *testing.T) {
	sentinel := errors.New("disk on fire")
	src := validDescriptor(t)
	err := saveDescriptorCache(&errMeta{err: sentinel}, src)
	if err == nil {
		t.Fatal("expected error")
	}
	if !errors.Is(err, sentinel) {
