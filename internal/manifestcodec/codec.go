...
// EncodeFileEncrypted produces the full file bytes for a manifest
// in MetadataOnly or Envelope mode per §3.3. Plain manifests must
// go through EncodeFile (refused here for symmetry with the
// dedicated Plain entry point).
//
// MetadataOnly: only the manifest's metadata field is encrypted.
// System fields (Namespace, BlobRef, Pipeline) stay in plaintext
// so Walk and routing work without DEK access.
//
// Envelope: the entire body bytes are encrypted as one ciphertext
// blob. ArtifactID becomes IV-dependent — two Encode calls on
// the same manifest produce different ArtifactIDs. Manifest-level
// dedup is intentionally lost in this mode (§3.3).
//
// AAD: the header bytes (magic + crypto flag + KeyID) are bound
// to the ciphertext via the AEAD tag. Tampering with KeyID or
// any header byte produces ErrDecryptionFailed at Open even
// before the engine recomputes ArtifactID.
