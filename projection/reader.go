...
type Reader interface {
	...
	Search(query string, limit int) []projection.SearchResult

	// RelatedByBlobRef returns artifacts sharing the given blob,
	// excluding the artifact named by exclude.
	RelatedByBlobRef(blobRef domain.BlobRef, exclude domain.ArtifactID) []projection.RelatedArtifact

	// LookupLocations returns every tree-placement of an artifact.
	LookupLocations(id domain.ArtifactID) (projection.Locations, bool)

	// StatsSnapshot returns a copy of the current counters.
	StatsSnapshot() projection.Stats
