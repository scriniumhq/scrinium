...
// to each position. Structurally equivalent to
// iter.Seq2[Node, error] from Go 1.23.
//
// On Go 1.23+ this becomes the standard alias:
//
//	type NodeSeq = iter.Seq2[Node, error]
//
// The current declaration is a local workaround for Go 1.22
// compatibility.
type NodeSeq func(yield func(Node, error) bool)
