...
func FuzzDecode(f *testing.F) {
	...
	f.Add([]byte(``))
	f.Add([]byte(`{}`))
	f.Add([]byte(`{"kind":"fs.v1","path":"a/b"}`))
	f.Add([]byte(`{"kind":"fs.v1","path":"../escape"}`))
	f.Add([]byte(`{"kind":"fs.v1","path":""}`))
	f.Add([]byte(`{"kind":"other-schema"}`))
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if !bytes.Equal(raw1, raw2) {
			t.Fatalf("encode not stable across reparse:\n %s\n %s", raw1, raw2)
		}
		if fs2.Path != fs.Path {
