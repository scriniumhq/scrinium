...
// faultyIndex wraps a real core.StoreIndex and injects errors into
...
func TestRecoverOrphans_TransientResolveError_PreservesBlob(t *testing.T) {
	...
	report, err := core.RecoverOrphans(context.Background(), drv, idx)
	if err != nil {
...
func TestRecoverOrphans_RemoveFails_OrphanStays(t *testing.T) {
	...
	report, err := core.RecoverOrphans(context.Background(), drv, indexfx.Memory(t))
	if err != nil {
		t.Fatalf("RecoverOrphans: %v", err)
	}
