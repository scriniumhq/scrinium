...
func TestSystemStore_Atomicity_FailBeforePointerFlip(t *testing.T) {
	...
	s := storefx.InitOn(t, realDrv, core.WithStoreIndex(idx))
	...
	s2 := storefx.OpenOn(t, faulty, core.WithStoreIndex(idx))
	defer s2.Close()
...
func TestSystemStore_Atomicity_FailBeforePredecessorDrop(t *testing.T) {
	...
	s := storefx.InitOn(t, realDrv, core.WithStoreIndex(indexfx.Memory(t)))
	ctx := context.Background()
