...
func TestWrapUnwrapDEK_RoundTrip(t *testing.T) {
	...
	wrapped, params, err := WrapDEK(dek, pass, config.KDFParams{})
	if err != nil {
...
func TestWrapDEK_DefaultsZeroCost(t *testing.T) {
	dek, _ := GenerateDEK()
	pass := []byte("xx")
	_, params, err := WrapDEK(dek, pass, config.KDFParams{})
	if err != nil {
		t.Fatal(err)
	}
	d := DefaultKDFParams()
...
func TestWrapDEK_RejectsBadCost(t *testing.T) {
	dek, _ := GenerateDEK()
	pass := []byte("xx")
	_, _, err := WrapDEK(dek, pass, config.KDFParams{Time: 0, Memory: 64 * 1024, Threads: 4})
...
func TestWrapDEK_RejectsBadDEKLength(t *testing.T) {
	pass := []byte("xx")
	_, _, err := WrapDEK([]byte{1, 2, 3}, pass, config.KDFParams{})
	if err == nil {
...
func TestUnwrapDEK_WrongPassphrase(t *testing.T) {
	dek, _ := GenerateDEK()
	wrapped, params, _ := WrapDEK(dek, []byte("right"), config.KDFParams{})
...
func TestUnwrapDEK_TamperedCiphertext(t *testing.T) {
	dek, _ := GenerateDEK()
	wrapped, params, _ := WrapDEK(dek, []byte("pass"), config.KDFParams{})
