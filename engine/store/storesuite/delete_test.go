...
func TestDelete_RemovesManifest(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			id, err := s.Put(context.Background(), artifactfx.Payload("delete me"))
			if err != nil {
				t.Fatal(err)
			}
...
func TestDelete_SharedBlobRefCount(t *testing.T) {
	...
	idA, err := s.Put(context.Background(), artifactfx.Payload(text),
		domain.WithSession("a"))
	if err != nil {
		t.Fatal(err)
	}
	idB, err := s.Put(context.Background(), artifactfx.Payload(text),
