// Model-based stateful test. A reference model (content the store should
// hold, keyed by the ArtifactID the store handed back) is run alongside the
// real store through a randomized program of operations. After every step the
// two are reconciled: every live artifact must read back its exact bytes, Walk
// must report exactly the live set, and the on-disk blob count must equal the
// number of distinct live contents (Plain dedup). Reopen is one of the
// operations, so persistence is exercised mid-stream, not just at the end.
//
...
//
//   - FuzzStoreModel decodes the program from the fuzz corpus, so the fuzz
//     minimizer shrinks any failing sequence to its core.
//   - TestStoreModel_Randomized runs a batch of fixed-seed programs so
//     `make test` (no -fuzz) still exercises the model every run.
//
// Plain mode only: the model predicts blob-level dedup (one blob per distinct
// live content), a Plain-mode law. It does NOT predict ArtifactID stability
// across re-Puts — the ID hashes a second-resolution timestamp, so identical
// content can yield a new id in a later second; the model keys on the id the
// store returns.
...
// distinctContents counts unique payloads across all live artifacts — the
// expected number of blobs on disk under Plain dedup. (Dedup keys on content
...
// runModelProgram interprets a byte program against a fresh Plain store and
// reconciles the model after every step. The byte stream is the only entropy
// source, so the run is fully reproducible from `program`.
func runModelProgram(t *testing.T, program []byte) {
	...
	for {
		...
		case opPut:
			...
			// A Put of content already live in this namespace may return the
			// SAME id (a same-second idempotent manifest) or a NEW id (a fresh
			// manifest stamped in a later wall-clock second). Both are valid:
			// the two artifacts share one deduped blob and both appear in Walk.
			// Mirror the store exactly — add a model entry only for an id we
			// have not seen. (Keying dedup on id rather than content is what
			// makes this race-safe; the blob dedup invariant is reconciled
			// separately on distinct content.)
			if m.findByID(id) < 0 {
...
// reconcileModel asserts the store agrees with the model on all observable
// surfaces: per-artifact content, the Walk set, and the distinct-blob count.
func reconcileModel(t *testing.T, s store.Store, root string, m *model, step int) {
	...
	// artifacts may linger until GC, so this is a lower-bound check on the
	// reachable set, not the physical file count — never fewer blobs than
	// distinct live contents.
...
// TestStoreModel_Randomized runs fixed-seed programs so the model is exercised
// on every `make test`, independent of fuzzing. Add a seed here when a fuzz
// hunt finds a sequence worth pinning as a permanent regression.
