...
// (escape analysis, GC compaction in future runtimes). Wipe is
// a best-effort hygiene measure, not a security guarantee. For
// threat models that require it (HSM-grade), use memguard-style
// locked memory in the host application.
//
...
//
// Lives in internal/manifestcrypto so both core (KEK/DEK
// lifecycle) and internal/manifestcodec (per-decode candidate
// keys) can share one definition without core depending on
// manifestcodec or vice versa.
func Wipe(b []byte) {
	clear(b)
}
