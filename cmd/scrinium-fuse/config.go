...
type Config struct {
	...
	IndexMode  string `yaml:"indexMode"` // "memory" | "persistent"
...
func bindFlags(fs *flag.FlagSet, cfg *Config) {
	...
	fs.StringVar(&cfg.MountPoint, "mount-point", cfg.MountPoint, "Filesystem mount point (required).")
	...
	fs.StringVar(&cfg.IndexMode, "index-mode", cfg.IndexMode, "Projection index mode: memory|persistent.")
	fs.BoolVar(&cfg.ReadOnly, "read-only", cfg.ReadOnly, "Mount read-only.")
}

// applyEnv overlays SCRINIUM_FUSE_* environment variables onto
// cfg. Only string-shaped fields are picked — env vars are blunt
// instruments and we do not want to expose every knob there.
func applyEnv(cfg *Config) {
	if v := os.Getenv("SCRINIUM_FUSE_STORE_PATH"); v != "" {
		cfg.StorePath = v
	}
